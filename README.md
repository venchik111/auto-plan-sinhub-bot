...
- **Расписание:** Skyeng Avatar API + импорт cookie-сессии без браузера на сервере
...
Открой [http://localhost:8000](http://localhost:8000) и войди в одобренный аккаунт. Форма Skyeng появляется автоматически: можно указать логин/email и пароль — пароль не сохраняется, сохраняется только сессия. Если Skyeng включит проверку «Я не робот» или запросит код подтверждения, используй запасной вариант и вставь значение заголовка `Cookie` из запроса `timetable/weekly` в DevTools браузера. Chromium на сервере не требуется.
...
Сессия Skyeng сохраняется отдельно для каждого аккаунта сайта. Расписание кэшируется на неделю, но при выборе другой недели загружается отдельный диапазон дат.

> Пароль Skyeng нужен только на время авторизации и не записывается в базу или файл. Cookie-сессия даёт доступ к расписанию пользователя: не записывай её в логи и используй HTTPS при публичном деплое.
...
| `Авторизация Skyeng устарела` | Снова скопируй Cookie из запроса `timetable/weekly` и переподключи расписание |
| `Skyeng включил проверку «Я не робот»` | Пройди вход в браузере и подключи расписание через Cookie — серверный вход не может пройти SmartCaptcha |
